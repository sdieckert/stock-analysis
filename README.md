...
Rather than loop through the data one stock and output to the sheet, the refactored code instead utilizes a tickerIndex and loops through capturing the needed information of totalVolume, endingPrice and startingPrice for each ticker and once all the data is stored for all stocks it then uses a loop to output the information onto the spreadsheet.
